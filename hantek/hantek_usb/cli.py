...
Ejecutar desde hantek/: python hantek_cli.py | python -m hantek_usb | python -m hantek_usb.cli
"""
...
def _cmd_write_settings(ns: argparse.Namespace) -> None:
    """dsoHTReadAllSet escritura: 10 B con [3]=0; opcional --tail (hasta 5 B en pos. 5–9)."""
    tail = b""
...
def _cmd_trig2(ns: argparse.Namespace, opcode: int) -> None:
    _cmd_04440(ns, opcode, ns.value & 0xFFFF, 2, not ns.no_wait)


def _cmd_trig_hpos(ns: argparse.Namespace) -> None:
    _cmd_04440(ns, Opcodes04440.TRIGGER_HPOS, ns.value & 0xFFFFFF, 3, not ns.no_wait)


def _cmd_trig_vpos(ns: argparse.Namespace) -> None:
    _cmd_04440(ns, Opcodes04440.TRIGGER_VPOS, ns.value & 0xFF, 1, not ns.no_wait)


def _cmd_ch1(ns: argparse.Namespace, sub: int) -> None:
    op = ch_opcode(ns.channel, sub)
    _cmd_04440(ns, op, ns.value & 0xFF, 1, not ns.no_wait)
...
_EPILOG = """Documentación:
  ../PROTOCOLO_USB.md       Visión general USB
  ../EXPORTS_HTHardDll.md   Cada export y bloques de 64 B esperados
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    add("scope-zero-cali", _cmd_scope_zero, help="dsoHTScopeZeroCali (solo escribe)")

    # --- DDS ---
    def _add_dds32(name: str, sub: int, h: str, *, value_help: str = "uint32 little-endian") -> None:
    ...
    _add_dds32("dds-fre", OpcodesDDS.FREQUENCY, "ddsSDKFre")
    _add_dds32("dds-amp", OpcodesDDS.AMP, "ddsSDKAmp")
    _add_dds32(
        "dds-offset",
        OpcodesDDS.OFFSET,
        "ddsSDKOffset. Escala empírica similar a dds-amp (1000≈1V en un equipo); "
        "el LCD puede no coincidir con la trama USB (ver HALLAZGOS_DMM_DDS_2026-03.md).",
        value_help="uint32 LE (misma familia que amplitud en pruebas)",
    )
    _add_dds32(
        "dds-square-duty",
    ...
    p = add("cmd04440", _cmd_cmd04440, help="Armar FUN_10004440 genérico")
