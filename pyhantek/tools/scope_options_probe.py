...
def _apply_sweep_step(
    ...
    *,
    ...
    v = int(value)
    if sweep == "time-div":
        pkt = fun_04440(Opcodes04440.TIME_DIV, v & 0xFFFF, 2, False)
    elif sweep == "ch1-volt":
        pkt = fun_04440(ch_opcode(0, 4), v & 0xFF, 1, False)
    elif sweep == "trigger-sweep":
        pkt = fun_04440(Opcodes04440.TRIGGER_SWEEP, v & 0xFFFF, 2, False)
    elif sweep == "trigger-source":
        pkt = fun_04440(Opcodes04440.TRIGGER_SOURCE, v & 0xFFFF, 2, False)
    elif sweep == "yt-format":
        pkt = fun_04440(Opcodes04440.YT_FORMAT, v & 0xFFFF, 2, False)
    else:
...
def _baseline_scope(link: HantekLink, time_div: int, after_op_s: float) -> None:
    ...
    link.write(fun_04440(Opcodes04440.TIME_DIV, int(time_div) & 0xFFFF, 2, False))
    link.write(fun_04440(Opcodes04440.TRIGGER_SOURCE, 0, 2, False))
    link.write(fun_04440(Opcodes04440.TRIGGER_SWEEP, 0, 2, False))
    link.write(scope_run_stop_stm32(True))
    time.sleep(0.12)
    if after_op_s > 0:
        time.sleep(after_op_s)
