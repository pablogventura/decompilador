...
Fuente del manual: [`../Hantek_2D72_2D42_Manual.txt`](../Hantek_2D72_2D42_Manual.txt) (extracto de PDF).  
Firmware de referencia: [`../../firmware/decompilado/`](../../firmware/decompilado/) (p. ej. `FUN_08031a9e`, `FUN_08032140`).  
DLL: [`EXPORTS_HTHardDll.md`](EXPORTS_HTHardDll.md), binario [`../../hantek/HTHardDll.dll`](../../hantek/HTHardDll.dll) y pseudocódigo en `decompiled_hantek/HTHardDll.dll/`.

Este archivo lista qué **falta implementar, probar en hardware o confirmar** respecto de lo que describe el manual y lo que ya existe en el CLI `hantek_usb`. Los huecos de **multímetro y generador** están resumidos en **§8** (checklist); el detalle validado sigue en [`../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md`](../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md).
...
- Captura **`0x16`**, **`read-settings`** (`0x15`) con `--parse`: `TIME_DIV` en `ram98_byte3`; flanco y barrido en `ram9c_byte3` / `ram9c_byte6` (ver [`../../pyhantek/hantek_usb/parse_resp.py`](../../pyhantek/hantek_usb/parse_resp.py) y [`../pyhantek/PROTOCOLO_USB.md`](../pyhantek/PROTOCOLO_USB.md)).
...
| **Force trigger (manual)** | Acción de UI para capturar sin disparo válido. **Trama USB desconocida** (≠ `0x17`). Procedimiento: [`../tools/PROCEDIMIENTO_force_trigger.txt`](../tools/PROCEDIMIENTO_force_trigger.txt) + [`../../pyhantek/tools/snapshot_scope_state.py`](../../pyhantek/tools/snapshot_scope_state.py) / [`../../pyhantek/tools/compare_scope_snapshots.py`](../../pyhantek/tools/compare_scope_snapshots.py). **UX 2D42 (empírico):** con disparo en **Normal**, Force en panel → sensación de **onda en movimiento** / poco respeto al trigger (parecido a **Auto** en el manual); con modo **Single**, el comportamiento encaja mejor con “una adquisición” del texto del PDF. |
...
7. **Diff de estado:** [`../../pyhantek/tools/compare_read_settings.py`](../../pyhantek/tools/compare_read_settings.py) y [`../../pyhantek/tools/snapshot_scope_state.py`](../../pyhantek/tools/snapshot_scope_state.py) para cerrar huecos (Force trigger, menús) sin adivinar bytes — **sin** usbmon/Wireshark (política del proyecto).
...
| **Horizontal System** (Y-T, Roll, Scan, X-Y, TIME) | `set-time-div`; **modo horizontal**: lectura `read-settings --parse` → `ram98_byte0`; **escritura** PC abierta ([`../pyhantek/PROTOCOLO_USB.md`](../pyhantek/PROTOCOLO_USB.md)). |
...
| Sección | USB / CLI |
|---------|-----------|
| Interface / Measurement | **Parcial:** `set-mode dmm`, `dmm-read`, `dmm-type`, etc. Detalle validado y huecos: **[`../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md`](../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md)**, **[`../pyhantek/DMM_FIRMWARE_DECODIFICACION.md`](../pyhantek/DMM_FIRMWARE_DECODIFICACION.md)**. Lista de pendientes: **§8** abajo. |

### Generator / AWG (manual p. ~22–26)

| Sección | USB / CLI |
|---------|-----------|
| Interface, sine, arb | **Parcial:** `set-mode dds`, `dds-wave`, `dds-fre`, `dds-amp`, …, `dds-download`. Eco IN poco fiable; cabecera download incompleta — **§8** y [`../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md`](../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md). |
...
Los comandos CLI existen; **no** dar por cerrada la paridad manual + pantalla + decodificación. Detalle ampliado en [`../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md`](../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md) (y DMM en [`../pyhantek/DMM_FIRMWARE_DECODIFICACION.md`](../pyhantek/DMM_FIRMWARE_DECODIFICACION.md)).
...
- [ ] **`dds-offset`:** discrepancia **USB vs LCD** y efecto en **salida real** (medir con DMM/scope); ver § offset en [`../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md`](../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md).
...
- [`../pyhantek/PROTOCOLO_USB.md`](../pyhantek/PROTOCOLO_USB.md) — protocolo USB y tablas empíricas.  
- `EXPORTS_HTHardDll.md` — mapeo DLL → opcodes.  
- [`../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md`](../pyhantek/HALLAZGOS_DMM_DDS_2026-03.md) — validado DMM/DDS + **pendientes enlazados desde §8**.  
- [`../pyhantek/DMM_FIRMWARE_DECODIFICACION.md`](../pyhantek/DMM_FIRMWARE_DECODIFICACION.md) — pipeline DMM en firmware.  
- [`../pyhantek/IMPLEMENTACION_CHECKLIST.md`](../pyhantek/IMPLEMENTACION_CHECKLIST.md) — checklist de pendientes con trazabilidad (decomp → código → test → hardware).  
- [`../../pyhantek/tools/scope_label_walk.py`](../../pyhantek/tools/scope_label_walk.py) — barridos interactivos para nuevas tablas.  
